...
// The number of rows of test data to create in the source databaseModel.
...
func testBackup(t *testing.T, testRowCount int, usePerPageSteps bool) {
	...
	// So generate a databaseModel driver name that will likely be unique.
	...
	// Connect to the source databaseModel.
	srcTempFilename := TempFilename(t)
	defer os.Remove(srcTempFilename)
	srcDb, err := sql.Open(driverName, srcTempFilename)
	if err != nil {
		t.Fatal("Failed to open the source databaseModel:", err)
	}
	defer srcDb.Close()
	err = srcDb.Ping()
	if err != nil {
		t.Fatal("Failed to connect to the source databaseModel:", err)
	}

	// Connect to the destination databaseModel.
	destTempFilename := TempFilename(t)
	defer os.Remove(destTempFilename)
	destDb, err := sql.Open(driverName, destTempFilename)
	if err != nil {
		t.Fatal("Failed to open the destination databaseModel:", err)
	}
	defer destDb.Close()
	err = destDb.Ping()
	if err != nil {
		t.Fatal("Failed to connect to the destination databaseModel:", err)
	}
	...
	if srcDbDriverConn == nil {
		t.Fatal("The source databaseModel driver connection is nil.")
	}
	destDbDriverConn := driverConns[1]
	if destDbDriverConn == nil {
		t.Fatal("The destination databaseModel driver connection is nil.")
	}
	...
	// Populate the source databaseModel with a test table containing some test data.
	tx, err := srcDb.Begin()
	if err != nil {
		t.Fatal("Failed to begin a transaction when populating the source databaseModel:", err)
	}
	_, err = srcDb.Exec("CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT)")
	if err != nil {
		tx.Rollback()
		t.Fatal("Failed to create the source databaseModel \"test\" table:", err)
	}
	for id := 0; id < testRowCount; id++ {
		_, err = srcDb.Exec("INSERT INTO test (id, value) VALUES (?, ?)", id, generateTestData(id))
		if err != nil {
			tx.Rollback()
			t.Fatal("Failed to insert a row into the source databaseModel \"test\" table:", err)
		}
	}
	err = tx.Commit()
	if err != nil {
		t.Fatal("Failed to populate the source databaseModel:", err)
	}

	// Confirm that the destination databaseModel is initially empty.
	...
	if destTableCount != 0 {
		t.Fatalf("The destination databaseModel is not empty; %v table(s) found.", destTableCount)
	}
	...
	if usePerPageSteps {
		...
		for {
			...
			if (time.Now().Unix() - startdate) > usePagePerStepsTimeoutSeconds {
	...
	// Confirm that the "test" table now exists in the destination databaseModel.
	...
	if err != nil {
		t.Fatal("Failed to check if the \"test\" table exists in the destination databaseModel:", err)
	}
	if !doesTestTableExist {
		t.Fatal("The \"test\" table could not be found in the destination databaseModel.")
	}

	// Confirm that the number of rows in the destination databaseModel's "test" table matches that of the source table.
	var actualTestTableRowCount int
	err = destDb.QueryRow("SELECT COUNT(*) FROM test").Scan(&actualTestTableRowCount)
	if err != nil {
		t.Fatal("Failed to determine the rowcount of the \"test\" table in the destination databaseModel:", err)
	}
	...
	// Check each of the rows in the destination databaseModel.
	for id := 0; id < testRowCount; id++ {
		var checkedValue string
		err = destDb.QueryRow("SELECT value FROM test WHERE id = ?", id).Scan(&checkedValue)
		if err != nil {
			t.Fatal("Failed to query the \"test\" table in the destination databaseModel:", err)
		}

		var expectedValue = generateTestData(id)
		if checkedValue != expectedValue {
			t.Fatalf("Unexpected value in the \"test\" table in the destination databaseModel; expected value: %v; actual value: %v", expectedValue, checkedValue)
		}
	}
}
