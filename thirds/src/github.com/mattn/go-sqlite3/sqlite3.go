...
// values into the databaseModel. When parsing a string from a timestamp or datetime
...
func (tx *SQLiteTx) Commit() error {
	...
	if err != nil && err.(Error).Code == C.SQLITE_BUSY {
		...
		// However, databaseModel/sql considers the transaction complete once we
...
// SQLITE_INSERT, SQLITE_DELETE, or SQLITE_UPDATE), the databaseModel name, the
...
func (c *SQLiteConn) RegisterFunc(name string, impl interface{}, pure bool) error {
	...
	// fi must outlast the databaseModel connection, or we'll have dangling pointers.
...
// Open databaseModel and return a new connection.
...
func (d *SQLiteDriver) Open(dsn string) (driver.Conn, error) {
	...
	if db == nil {
		return nil, errors.New("sqlite succeeded without returning a databaseModel")
	}
...
func (s *SQLiteStmt) Close() error {
	...
	s.closed = true
	if !s.c.dbConnOpen() {
		return errors.New("sqlite statement with already closed databaseModel connection")
	}
